        ...
        if (is_writable($f2)) {
            if (is_writable($f3)) {
                ...
                foreach ($tab as $t) {
                    ...
                    $index = $t['index'];
                    $crea_doc = 'mkdir "' . $donner . $chemin . '" ' . $crlf .'icacls "' . $donner . $chemin . '" /inheritance:r /t ' . $crlf . 'icacls "' 
                    . $donner . $chemin . '" /grant GL_RO_' . $index . ':R ' . $crlf . 'icacls "' . $donner . $chemin . '" /grant GL_RW_' . $index . ':R ' . $crlf;
                    file_put_contents($f, $crea_doc, FILE_APPEND);
                    chemin($chemin, $t['id_dos'], $f, $donner);
                    $crea_groupes = 'net group "GG_RW_' . $index . '" /add  ' . $crlf . 'net localgroup "GL_RW_' . $index . '" /add ' 
                    . $crlf . 'net group "GG_RO_' . $index . '" /add ' . $crlf . 'net localgroup "GL_RO_' . $index . '" /add ' . $crlf . 'net localgroup "GL_RO_' 
                    . $index . '" "GG_RO_' . $index . '" /add ' . $crlf . 'net localgroup "GL_RW_' . $index . '" "GG_RW_' . $index . '" /add ' . $crlf;
                ...
                $ta = pg_fetch_all($re);
                foreach ($ta as $l) {
                    $s = 'select dossier.index as index, account.login as login, id_l,id_e from attribution join dossier on attribution.id_dos = dossier.id_dos 
                    join account on attribution.id_ac = account.id_ac where  attribution.id_ac = ' . $l['id_ac'];
                    $re = pg_query($s);
                    $ta = pg_fetch_all($re);
                    $i = 0;
                    foreach ($ta as $f) {
...
function chemin($chemin, $id, $fichier,$d)
{
    ...
    foreach ($tab2 as $l) {
        ...
        $ch = 'mkdir "' . $d . $chemin2 . '" ' . $crlf . 'icacls "' . $d . $chemin2 . '" /inheritance:r /t ' . $crlf . 'icacls "' . $d 
        . $chemin2 . '" /grant GL_RO_' . $index . ':R ' . $crlf . 'icacls "' . $d . $chemin2 . '" /grant GL_RW_' . $index . ':R ' . $crlf;
...
function group($id, $fichier)
{
    ...
    foreach ($tab2 as $l) {
        ...
        $ch = 'net group "GG_RW_' . $index . '" /add' . $crlf . 'net localgroup "GL_RW_' . $index . '" /add ' . $crlf . 'net group "GG_RO_' 
        . $index . '" /add ' . $crlf . 'net localgroup "GL_RO_' . $index . '" /add ' . $crlf . 'net localgroup "GL_RO_' . $index . '" "GG_RO_' 
        . $index . '" /add ' . $crlf . 'net localgroup "GL_RW_' . $index . '" "GG_RW_' . $index . '" /add ' . $crlf . 'net localgroup "GL_RO_' . $tab[0]['index'] 
        . '" "GG_RO_' . $index . '" /add ' . $crlf . 'net localgroup "GL_RO_' . $tab[0]['index'] . '" "GG_RW_' . $index . '" /add ' . $crlf;
        file_put_contents($fichier, $ch, FILE_APPEND);
